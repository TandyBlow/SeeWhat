// Shared helper functions used by both 2D and 3D noise
...
export const SIMPLEX_3D = NOISE_HELPERS + /* glsl */ `
float snoise(vec3 v) {
...
// Simplex 2D noise
export const SIMPLEX_2D = NOISE_HELPERS + /* glsl */ `
float snoise(vec2 v) {
...
// Combined: includes helpers once + both noise functions
export const NOISE_ALL = NOISE_HELPERS + /* glsl */ `
float snoise(vec3 v) {
  const vec2 C = vec2(1.0/6.0, 1.0/3.0);
  const vec4 D = vec4(0.0, 0.5, 1.0, 2.0);
